...
def ghost_chase_rand3(walls):
    ...
    :return: none
    """
    global ghost_speeds, ghost_x3, ghost_y3, pac_x, pac_y, tile_width, prev_cutoff_3, ghost_directions
    ...
    if walls != "null":
        ...
        # check if column is equal to row and restrict equidistant ghost motion using related lists
        # if delta_column == delta_row:
        #     prev_cutoff_1 = []
        #     equidistant = True
        #     for z in range(len(ghost3_poss_speeds)):
        #         if ghost3_poss_speeds[z] == 0:
        #             prev_cutoff_1.append(ghost_directions[z])
        ...
        # check if column is equal to row and restrict equidistant ghost motion using related lists
        if delta_column == delta_row:
            # check the direction the ghost is moving in; do NOT allow motion back in there
            if ghost_speeds[2][0] != 0:
                # moving right, should not go left
                if ghost_speeds[2][0] > 0:
                    prev_cutoff_3.append("left")
                else:
                    # moving left, not go back right
                    prev_cutoff_3.append("right")
            elif ghost_speeds[2][1] != 0:
                # moving up, should not go down
                if ghost_speeds[2][1] > 0:
                    prev_cutoff_3.append("down")
                else:
                    # moving down, not go back up
                    prev_cutoff_3.append("up")
            # set error status
            motion_error = True
    # move the ghost

...
def on_update(delta_time):
    ...
    global score, ghost_speeds, ghost_rand_x, ghost_rand_y, lose, win, row_count, column_count, lives
...
def write_lives(lives):
    """ write the number of lives which the player has

    :param lives: number of lives (integer)
    :return: none
    """
    arcade.draw_text("LIVES: " + str(lives), 400, 620, arcade.color.UNIVERSITY_OF_TENNESSEE_ORANGE, 30)
...
def draw_menu(texture):
    ...
    :param texture: image file to output to screen
    :return: none
    """
    global WIDTH, HEIGHT
    # draw the menu
...
def draw_time_num(time):
    ...
    arcade.draw_text(str(round_time), 1025, 630, arcade.color.ORANGE, 20)
...
def pac_win_lose(x, y):
    ...
    :return: none
    """
    global pac_speed_x, pac_speed_y, init_arc_angle, final_arc_angle, ghost_speeds, win, lose, tile_width, lives
    # check if pacman is in contact with any ghost
    distance1 = ((ghost_x1 - x) ** 2 + (ghost_y1 - y) ** 2) ** (1 / 2)
    distance2 = ((ghost_x2 - x) ** 2 + (ghost_y2 - y) ** 2) ** (1 / 2)
    distance3 = ((ghost_x3 - x) ** 2 + (ghost_y3 - y) ** 2) ** (1 / 2)
